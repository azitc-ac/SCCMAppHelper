...
| **Edit** | Opens the application: the facts on top, the definition below. A package that exists is rebuilt from the edited row, so the row stays the source of truth. Only for rows with a definition - a `Foreign` or `Ready to import` folder is imported first (Build / Import), which reads its script into a row |
...
Management Studio package measures 259 characters under `\\LAB01.lab.example` and 265 under
`\\CMSERVER.customer.example`. The same package, from the same tool, publishes on one site and not
on the other.
...
"activeSite": "",
"sites": [
    { "name": "Customer (P01)", "siteCode": "P01", "siteServer": "CMSERVER", ... },
