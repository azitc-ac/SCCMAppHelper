...
applications built from the master list, published, deployed to `CLIENT01` and installed there,
...
and the step in the log says "Building package", the same word as the button. The two rows it is for say so as their status: `Ready to build` (was `Definition only`) and `Ready to import` (was `Ready to publish`, which hid that the folder had no row). Edit and New version are enabled only for rows with a definition: editing a Foreign package before importing it opened an empty editor, and OK would have rebuilt the package over the commands its script carries. Double-click on such a row shows the facts with a note pointing to Build / Import; after the import the status is `Imported, publish pending`, and Edit is open. Edge on the lab share showed the gap: a row from the older scripts next to their package (no tool blocks, published by them) - status Foreign, Edit open, Build greyed out, so nothing could take it over. Build / Import is now enabled for any package without the tool's blocks, row or not, and goes through Import, which merges the script's sections with the row. Second try on Edge: still Foreign - the import had written nothing into the script, because an MSI row has no commands and Import only wrote what the row said. Import now writes the MSI Install/Uninstall blocks the way a build does; Edge: two tool blocks, `Imported, publish pending`. New version keeps
...
cloud session could only parse or fake has now been executed on LAB01.
...
The ConfigMgr layer has now been run against a real site. Everything in the L01 section below
was executed on the lab site on LAB01 on 2026-08-28, under Windows PowerShell 5.1 with the
...
### Verified on the L01 lab site (LAB01, 2026-08-28)
...
* `New-CMDeviceCollection` with `New-CMSchedule`, plus `Move-CMObject` and `New-CMFolderPath`
  ...
  `L01:\DeviceCollection\Deployment\Software` does not list them even when they are there.
* `Start-CMContentDistribution` and `Update-CMDistributionPoint` - content for both test
  applications is on `\\LAB01.lab.example`, confirmed through `SMS_DPContentInfo`
...
* `Update-OutdatedAppsCollection` - the SQL query runs against `CM_L01`. The joins and the
...
### Verified on the L01 lab site (LAB01, 2026-08-28, native detection)
...
### The whole chain, end to end (L01, 2026-08-28)
...
### File detection: `Notepad++ - 8.9.8` (L01, 2026-08-28)
...
### Publishing what the older scripts built (L01, 2026-08-29)
...
### The retire workflow, built and run (L01, 2026-08-29)
...
### A detection evaluated on a real client (CLIENT01, 2026-08-31)
...
and deployed to `CLIENT01.lab.example`, which had 26.00 installed.
...
`Notepad++ - 8.9.8` was deployed to CLIENT01, which had no Notepad++ at all. `AppEnforce.log`:
...
**Replacing a legacy script detection with a clause works.** `Customer application - 20.25.0`
...
### Five applications deployed to a client, and what stood in the way (CLIENT01, 2026-08-31)

Five applications were built with the tool, published, and deployed to `CLIENT01`. All five are
...
1. **The install command carried the wrong verb.** `vs_SSMS.exe` is a Visual Studio
   ...
   `AppEnforce` beyond the code. `CLIENT01` already carried SSMS 22.3 in that path. A version change
...
* **`Notepad++ - 8.9.8` and `7-Zip - 26.02` are real, not test objects**, and both are now
  deployed and installed on `CLIENT01` along with PDF24, Remote Desktop Manager and SSMS. They
  should not be swept up by any lab cleanup.
* **Notepad++ is installed twice on `CLIENT01`** - an EXE and an MSI installation side by side,
...
* **The customer site hit the same limit on 2026-09-01**, on a package that publishes here
  without complaint: the SSMS layout measures 259 characters under `\\LAB01.lab.example` and 265
  under `\\CMSERVER.customer.example`. Four files over, purely because the server name is six
