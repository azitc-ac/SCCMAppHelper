...
and the step in the log says "Building package", the same word as the button. The two rows it is for say so as their status: `Ready to build` (was `Definition only`) and `Ready to import` (was `Ready to publish`, which hid that the folder had no row). Edit and New version are enabled only for rows with a definition: editing a Foreign package before importing it opened an empty editor, and OK would have rebuilt the package over the commands its script carries. Double-click on such a row shows the facts with a note pointing to Build / Import; after the import the status is `Imported, publish pending`, and Edit is open. Edge on the lab share showed the gap: a row from the older scripts next to their package (no tool blocks, published by them) - status Foreign, Edit open, Build greyed out, so nothing could take it over. Build / Import is now enabled for any package without the tool's blocks, row or not, and goes through Import, which merges the script's sections with the row. New version keeps
